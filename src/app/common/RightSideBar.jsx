...
export default function RightSideBar() {
  ...
  let handleincrementviews=(videoId)=>{
    axios.post(`http://localhost:5000/api/video/increment-views?id=${videoId}`)
    .then((response)=>{
      if(response.data.status){
        toast.success(response.data.msg)
                    ...
                    <Link key={index} href={`/watch/${video._id}`}>
                      <div className="border border-gray-300 lg:p-2 lg:m-2 rounded-lg h-[330px]" onClick={()=>handleincrementviews(video._id)}>
                        ...
                        <div className="flex gap-2 p-2">
                          <img src={"http://localhost:5000/uploads/users/" + video.videouploader.image} alt={video.videouploader.name} className="w-10 h-10 rounded-full" />
                          <div>
