...
export default function WatchLater({ id }) {
  ...
  useEffect(() => {
    axios.post(`https://youtube-server-a5ha.onrender.com/api/video/watch-later/view-all-videos-watch-later?id=${id}`)
      .then((response) => {
  ...
  let handleDeleteVideosInWatchLater = (VideoId) => {
    axios.post(`https://youtube-server-a5ha.onrender.com/api/video/watch-later/update-watch-later?id=${id}`, {
      videoid: VideoId
    })
      .then((response) => {
        if(response.data.status==true){
