...
export default function PlaylistVideos() {
  ...
  useEffect(() => {
    axios.post(`http://localhost:5000/api/video/view-all-videos-in-playlist?id=${id}`)
      .then((response) => {
  ...
  let deleteVideoInPlaylist = (videoid) => {
    let playlistid = playlist._id
    axios.post(`http://localhost:5000/api/video/delete-video-in-playlist?id=${playlistid}`, {
      videoid: videoid
    })
      .then((response) => {
        if (response.data.status == true) {
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="p-3.5">
                  ...
                  <div className="flex gap-5 items-center p-2">
                    <img src={"http://localhost:5000/uploads/users/" + user.image} alt="" className='max-w-25 max-h-25 w-full rounded-full' />
                    <div>
                      ...
                      return (
                        <Link href={`/watch/${video._id}`}>
                          <div className="grid grid-cols-[30%_auto] gap-3 border p-3 rounded-xl border-gray-800 hover:scale-105 hover:shadow-gray-600 shadow-md relative" key={index}>
                            ...
                            <img src={"http://localhost:5000/uploads/videos/thumbnails/" + video.thumbnail} alt="" className='h-full rounded-lg' />
                            <div className="">
              ...
              videos.map((v, i) => {
                return (
                  <div key={i} className="border border-gray-300 lg:p-1 rounded-lg h-[200px]" onClick={() => handleaddvideos(v._id)}>
                    <img src={"http://localhost:5000/uploads/videos/thumbnails/" + v.thumbnail} alt={v.videotitle} className="w-full object-cover rounded-md mb-2" />
                    <div className="flex gap-2 p-2">
                      <img src={"http://localhost:5000/uploads/users/" + v.videouploader.image} alt={v.videouploader.name} className="w-8 h-8 rounded-full" />
                      <div>
