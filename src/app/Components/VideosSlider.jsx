...
export default function VideosSlider({ id }) {
    ...
    let addToWatchlater = (videoid) => {
        ...
        axios.post(`https://youtube-server-all.up.railway.app/api/video/watch-later/add-video-watch-later?id=${id}`,{
    ...
    let deleteVideo=(deleteVideoId)=>{
        if(confirm("Are you sure want to delete this video ?")){
            axios.post(`https://youtube-server-all.up.railway.app/api/video/delete-video/${deleteVideoId}`)
            .then((response)=>{
                if(response.data.status==true){
                    toast.success(response.data.msg)
