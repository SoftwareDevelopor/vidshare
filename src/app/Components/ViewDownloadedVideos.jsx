...
export default function ViewDownloadedVideos() {
    ...
    const fetchDownloadedVideos = async () => {
        try {
            setLoading(true)
            const response = await axios.post(`https://youtube-server-a5ha.onrender.com/api/video/download-video/view?id=${userid}`)
    ...
    const fetchVideoDetails = async (downloads) => {
        try {
            const videosDetailPromises = downloads.map(download =>
                axios.post(`https://youtube-server-a5ha.onrender.com/api/video/view-video?id=${download.videoid}`)
                    .then(res => {
    ...
    const handleDownloadVideo = async (videoId, videoFile) => {
        try {
            ...
            const url = `https://youtube-server-a5ha.onrender.com/api/video/download-video/create?id=${userid}`
                        ...
                        {videosWithDetails.map((video) => (
                            ...
                            >
                                ...
                                <div className="relative h-48 bg-gray-200 dark:bg-gray-700 overflow-hidden">
                                    {video.thumbnail ? (
                                        <img
                                            src={`https://youtube-server-a5ha.onrender.com/uploads/videos/thumbnails/${video.thumbnail}`}
